...
export default function Chat({ clientId, isAdmin, clientNom }) {
  ...
  async function loadMessages() {
    ...
    setMessages(data || []);
    await supabase.from("messages").update({ lu: true }).eq("client_id", clientId).eq("auteur", isAdmin ? "client" : "admin");
    setLoading(false);
  }
  ...
  const formatDate = (dateStr) => new Date(dateStr).toLocaleDateString("fr-FR", { day: "numeric", month: "long" });
            ...
            <div key={msg.id}>
              ...
              <div style={{ display: "flex", justifyContent: isMine ? "flex-end" : "flex-start" }}>
                <div style={{
                  ...
                  color: isMine ? "#fff" : "#1C1C1E", fontSize: 13, lineHeight: 1.5
                }}>
                  {!isMine && (
